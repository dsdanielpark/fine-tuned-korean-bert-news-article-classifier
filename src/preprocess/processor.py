...
class NLPdata:
    ...
    def _get_label_encoder(
        self, df: pd.DataFrame, labelCol: str, reverse: bool
    ) -> pd.DataFrame:
    ...
    def get_corpus_cluster_df(self, corpus: list, num_clusters: int) -> list:
        ...
        clustered_list = [
            [str(i) + "cluster" + x for x in clustered_corpus[i]]
    ...
    def get_df_cluster(self, df: pd.DataFrame, num_clusters: int) -> pd.DataFrame:
        ...
        df_cluster["cluster"] = [
            int(x.split("cluster")[0]) for x in df_cluster["cleanBody"]
        ]
        df_cluster["cleanBody"] = [
            x.split("cluster")[1] for x in df_cluster["cleanBody"]
        ]
        df_cluster["index"] = [
            int(x.split("index")[0]) for x in df_cluster["cleanBody"]
        ]
    ...
    def TSVDdataset(
        self, file_path: str, bodyCol: str, labelCol: str, mode: str, num_cluster=None
    ) -> list:
        ...
        if mode == "mode1":
            ...
            df["cleanBody"] = [
                f"기사제목:: {t}      기사본문: {b}"
                for t, b in zip(df["title"], df["cleanBody"])
            ]
            TVSD_dataset = self.make_TSVD_list(df, bodyCol, labelCol)
        elif mode == "mode4":
            df_clustered = self.get_df_cluster(df, num_cluster)
            df_clustered["cleanBody"] = [
                f"기사제목: {t}     기사본문: {b}"
                for t, b in zip(df_clustered["title"], df_clustered["cleanBody"])
            ]
